<?php
	...
	$comando = "UPDATE fabricante SET nome='$nome', descricao='$descricao' WHERE id='$id'";
	...
	if (count($quantidade) > 0) {
	    ...
	    $fabricante = mysql_fetch_assoc($consulta);
	} else {
	    //se deu erro
...
<html>
	...
	<form action="editar_fabricante.php" method="post">
	    
	    <input type="hidden" name="id" value="<?php echo $fabricante['id']; ?>" />
	    
	    
	    <label for="nome">Nome</label>
	    <input type="text" name="nome" id="nome" value="<?php echo $fabricante['nome']; ?>" />
	    
	    <br />
	    
	    <label for="descricao">Descrição</label>
	    <textarea name="descricao" id="descricao" ><?php echo $fabricante['descricao']; ?></textarea>
